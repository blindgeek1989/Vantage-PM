...
function activateLicense(){const key=document.getElementById('license-key-input')?.value.trim();if(!key){announce('Enter a license key.');return;}saveSetting('licenseKey',key);toast('License key saved. Online activation coming soon.','success');announce('License key saved. Activation coming soon.');}
...
async function submitFeedbackForm(){const issue=document.getElementById('fb-issue').value.trim();const impact=document.getElementById('fb-impact').value;let ok=true;if(!issue){document.getElementById('fb-issue-err').classList.remove('hidden');ok=false;}else document.getElementById('fb-issue-err').classList.add('hidden');if(!impact){document.getElementById('fb-impact-err').classList.remove('hidden');ok=false;}else document.getElementById('fb-impact-err').classList.add('hidden');if(!ok){announce('Please fix the errors above.');return;}const at=[...document.querySelectorAll('input[name="fb-at"]:checked')].map(c=>c.value);const solution=document.getElementById('fb-solution').value.trim();const btn=document.getElementById('fb-submit');btn.disabled=true;btn.textContent='Submitting…';const _ftitle=encodeURIComponent('[Feedback] '+issue.slice(0,72));const _fbody=encodeURIComponent('**AT:** '+(at.join(', ')||'Not specified')+'\n\n**Issue:** '+issue+'\n\n**Suggested solution:** '+(solution||'N/A')+'\n\n**Impact:** '+impact);window.open('https://github.com/blindgeek1989/Vantage-PM/issues/new?title='+_ftitle+'&body='+_fbody+'&labels=user-feedback','_blank');btn.disabled=false;btn.textContent='Submit Feedback';closeModal('feedback-modal');toast('GitHub opened — complete the submission there.','success');announce('GitHub opened to submit your feedback.');}
