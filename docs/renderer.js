...
// Update this URL once the Cloudflare Worker is deployed
const WORKER_URL = 'https://vantagepm-license.blindgeek1989.workers.dev';
...
const DEFAULT_SETTINGS = {
  ...
  focusModeUser:'',hasSeenOnboarding:false,appMode:'',
  shortcuts:{
  ...
  googleConnected:false,googleEmail:null,platform:'web',appVersion:'9.9.2',agingThresholdDays:5,
  tier:'free',licenseKey:'',
};
...
async function init(){
  ...
  const _s=await AppStorage.loadSettings();
  if(_s)applySettings(_s);
  const _d=await AppStorage.loadData();
  if(_d)applyDataSnapshot(_d);
  if(typeof SupaAuth!=='undefined'){
    try{
      const{data:{session}}=await SupaAuth.getSession();
      if(session){_supaUser=session.user;const remote=await SupaData.load();if(remote){if(remote.settings)applySettings(remote.settings);if(remote.data)applyDataSnapshot(remote.data);if(remote.settings)await AppStorage.saveSettings(remote.settings);if(remote.data)await AppStorage.saveData(remote.data);}}
    }catch(e){console.warn('Supabase init:',e);}
    SupaAuth.onAuthStateChange(async(event,session)=>{
      if(event==='SIGNED_IN'&&session){
        _supaUser=session.user;
        try{const remote=await SupaData.load();if(remote){if(remote.settings)applySettings(remote.settings);if(remote.data)applyDataSnapshot(remote.data);if(remote.settings)await AppStorage.saveSettings(remote.settings);if(remote.data)await AppStorage.saveData(remote.data);}}catch(e){console.warn('Supabase SIGNED_IN sync:',e);}
        if(activeSettingsTab==='account'){const body=document.getElementById('settings-panel-body');if(body)body.innerHTML=renderAccount();}
        announce('Signed in successfully.');toast('Signed in.','success');
      }else if(event==='SIGNED_OUT'){
        _supaUser=null;
        if(activeSettingsTab==='account'){const body=document.getElementById('settings-panel-body');if(body)body.innerHTML=renderAccount();}
        announce('Signed out.');toast('Signed out.');
      }
    });
  }
  window.matchMedia('(prefers-color-scheme:dark)').addEventListener('change',e=>{systemDark=e.matches;applyTheme();});
...
function isProUser(){return settings.tier==='pro'||settings.tier==='legacy';}
function showUpgradePrompt(featureName){const el=document.getElementById('upgrade-feature-name');if(el)el.textContent=featureName;openModalEl('upgrade-modal');announce(`${featureName} is a Pro feature.`);}

function applySettings(s){Object.assign(settings,s);applyTheme();document.documentElement.lang=settings.language;document.documentElement.style.setProperty('--fs',settings.fontSize+'px');if(settings.reduceMotion)document.documentElement.setAttribute('data-reduce-motion','');else document.documentElement.removeAttribute('data-reduce-motion');if(settings.highContrast)document.documentElement.setAttribute('data-contrast','');else document.documentElement.removeAttribute('data-contrast');applyAppMode();}
function applyAppMode(){const isPM=settings.appMode==='pm';const auditLi=document.querySelector('[data-view=audit]')?.closest('li');if(auditLi)auditLi.hidden=isPM;if(isPM&&currentView==='audit')nav('dashboard',document.querySelector('[data-view=dashboard]'));}

const MORE_VIEWS=['milestones','health','templates','activity','whats-new','onboarding'];
function toggleMoreNav(){
  moreNavOpen=!moreNavOpen;
  const sec=document.getElementById('more-nav-section');
  const btn=document.getElementById('more-nav-btn');
  if(sec) sec.hidden=!moreNavOpen;
  if(btn){
    btn.setAttribute('aria-expanded',String(moreNavOpen));
    const arrow=btn.querySelector('.more-nav-arrow');
    if(arrow) arrow.style.transform=moreNavOpen?'rotate(180deg)':'rotate(0deg)';
  }
  announce(moreNavOpen?'More navigation items expanded.':'More navigation items collapsed.');
}
...
function toggleActionsMenu(){
  ...
  if(isOpen){closeActionsMenu();return;}
  menu.hidden=false;
  btn.setAttribute('aria-expanded','true');
  menu.onkeydown=ev=>{
    if(ev.key==='Escape'){ev.preventDefault();closeActionsMenu();btn.focus();return;}
...
function applyDataSnapshot(data){
  ...
  if(data.appMode)settings.appMode=data.appMode;
}
let _localSaveTimer=null,_supaDataTimer=null,_supaSettingsTimer=null;
function scheduleLocalSave(){
  clearTimeout(_localSaveTimer);
  _localSaveTimer=setTimeout(()=>{AppStorage.saveData({tasks,members,projects,milestones,activityLog,auditResults,customStatuses,templates,projectNotes,filterPresets,persona:settings.persona,appMode:settings.appMode});scheduleSupaSave();},800);
}
function scheduleSupaSave(){if(typeof SupaData==='undefined'||!_supaUser)return;clearTimeout(_supaDataTimer);_supaDataTimer=setTimeout(()=>SupaData.saveData({tasks,members,projects,milestones,activityLog,auditResults,customStatuses,templates,projectNotes,filterPresets,persona:settings.persona,appMode:settings.appMode}),3000);}
function scheduleSupaSettingsSave(){if(typeof SupaData==='undefined'||!_supaUser)return;clearTimeout(_supaSettingsTimer);_supaSettingsTimer=setTimeout(()=>SupaData.saveSettings(Object.assign({},settings)),3000);}

function checkReminders(){if(!settings.notifications)return;const lead=settings.reminderLeadDays||1;const cutoff=new Date();cutoff.setDate(cutoff.getDate()+lead);tasks.forEach(t=>{if(t.status==='done')return;const due=new Date(t.due+'T00:00');if(due<=cutoff)sendWebNotification(due<today?'Overdue':'Upcoming',`"${t.name}" ${due<today?'is overdue':'is due '+fmtDate(t.due)}.`);});}
function sendWebNotification(title,body){if(!('Notification' in window))return;if(Notification.permission==='granted'){new Notification(title,{body,icon:'assets/icon.png'});}else if(Notification.permission!=='denied'){Notification.requestPermission().then(p=>{if(p==='granted')new Notification(title,{body,icon:'assets/icon.png'});});}}
...
function updateContextualActions(view){
  const ca=document.getElementById('contextual-actions');
  if(!ca) return;
  if(view==='audit') ca.innerHTML=`<button class="btn btn-secondary btn-sm" onclick="exportAuditReport()"><span aria-hidden="true">🖨️</span> Print Report</button>`;
  else if(view==='tasks') ca.innerHTML=`<button class="btn btn-secondary btn-sm" onclick="importCSVClick()"><span aria-hidden="true">📤</span> Import CSV</button>`;
  else ca.innerHTML='';
}

function nav(view,btn,settingsTab){
  if(view==='audit'&&settings.appMode==='pm'){announce('WCAG Audit is not available in Project Management mode.');return;}
  currentView=view;
  if(MORE_VIEWS.includes(view)&&!moreNavOpen){
    moreNavOpen=true;
    const sec=document.getElementById('more-nav-section');
    const mb=document.getElementById('more-nav-btn');
    if(sec) sec.hidden=false;
    if(mb){mb.setAttribute('aria-expanded','true');const arr=mb.querySelector('.more-nav-arrow');if(arr)arr.style.transform='rotate(180deg)';}
  }
...
function renderDashboard(){
  ...
  let myT=focusMode?tasks.filter(t=>t.assignee===(settings.focusModeUser||members[0]?.name)):tasks;
  const ts=today.toISOString().split('T')[0];
  const overdueTasks=myT.filter(t=>t.status!=='done'&&new Date(t.due+'T00:00')<today).sort((a,b)=>new Date(a.due)-new Date(b.due));
  const todayTasks=myT.filter(t=>t.due===ts&&t.status!=='done');
  const upMs=milestones.filter(m=>{const d=new Date(m.date+'T00:00');return d>=today&&d<=new Date(today.getTime()+14*86400000);}).sort((a,b)=>new Date(a.date)-new Date(b.date));
  const taskRow=t=>`<li class="today-task-item">${isTaskAging(t)?`<span class="badge badge-aging" aria-label="${taskAgeDays(t)} days stale"><span aria-hidden="true">🕐</span></span> `:''}${sBadge(t.status)}<span style="flex:1;font-weight:600">${esc(t.name)}</span>${t.assignee?`<span style="font-size:.78rem;color:var(--muted)">${esc(t.assignee)}</span>`:''}  <button class="btn btn-success btn-sm" onclick="markDone(${t.id})" aria-label="Mark ${esc(t.name)} done">Done</button><button class="btn btn-secondary btn-sm" onclick="openEditTask(${t.id})" aria-label="Edit ${esc(t.name)}">Edit</button></li>`;
  return`${_setupNotice}
<section class="today-widget" role="region" aria-label="Needs attention">
  <h2 style="font-size:.9rem;font-weight:700;color:var(--accent);margin-bottom:12px"><span aria-hidden="true">🎯</span> Needs Attention — ${today.toLocaleDateString(settings.language,{weekday:'long',month:'long',day:'numeric'})}</h2>
  ${overdueTasks.length?`<p style="font-size:.78rem;font-weight:700;color:var(--danger);margin-bottom:6px;text-transform:uppercase;letter-spacing:.4px"><span aria-hidden="true">⚠️</span> Overdue (${overdueTasks.length})</p><ul style="list-style:none;padding:0;margin:0 0 14px">${overdueTasks.map(taskRow).join('')}</ul>`:''}
  <p style="font-size:.78rem;font-weight:700;color:var(--muted);margin-bottom:6px;text-transform:uppercase;letter-spacing:.4px"><span aria-hidden="true">📅</span> Due Today (${todayTasks.length})</p>
  ${todayTasks.length?`<ul style="list-style:none;padding:0;margin:0">${todayTasks.map(taskRow).join('')}</ul>`:'<p style="font-size:.85rem;color:var(--muted)">Nothing due today.</p>'}
</section>
${upMs.length?`<section class="card mt-16" role="region" aria-label="Upcoming milestones"><h2 class="section-h"><span aria-hidden="true">🏁</span> Upcoming Milestones</h2><ul style="list-style:none;padding:0">${upMs.map(m=>{const proj=projects.find(p=>p.id===m.projectId);return`<li class="milestone-item"><span aria-hidden="true">🏁</span><span style="flex:1;font-weight:600">${esc(m.name)}</span>${proj?`<span class="project-chip" style="color:${proj.color};border-color:${proj.color}">${esc(proj.name)}</span>`:''}<span style="font-size:.82rem;color:var(--muted)">${fmtDate(m.date)}</span></li>`;}).join('')}</ul></section>`:''}`;
}
...
function saveProject(){if(!isProUser()&&projects.length>=3){showUpgradePrompt('Unlimited Projects (beyond 3)');return;}const name=document.getElementById('p-name').value.trim();if(!name){announce('Enter a project name.');return;}const newProj={id:nextProjectId++,name,desc:document.getElementById('p-desc').value.trim(),color:document.getElementById('p-color').value};projects.push(newProj);logActivity(`Project "${name}" created`,'📁');closeModal('project-modal');if(_addProjectFromTask){_addProjectFromTask=false;populateProjectSelect(newProj.id);announce(`Project "${name}" created and selected.`);toast(`Project "${name}" added.`,'success');}else{nav('projects',document.querySelector('[data-view=projects]'));announce('Project added.');toast('Project added.','success');}}
...
function openAddMilestone(){if(!isProUser()){showUpgradePrompt('Milestones');return;}document.getElementById('ml-project').innerHTML='<option value="">— No project —</option>'+projects.map(p=>`<option value="${p.id}">${esc(p.name)}</option>`).join('');openModalEl('milestone-modal');}
...
function exportAuditCSV(){if(!isProUser()){showUpgradePrompt('WCAG Audit Export');return;}const headers=['ID','Name','Level','Status','Notes'];const rows=WCAG_CRITERIA.map(c=>{const r=auditResults[c.id]||{};return[c.id,c.name,c.level,r.status||'not set',r.note||''];});const csv=[headers,...rows].map(r=>r.map(v=>`"${String(v).replace(/"/g,'""')}"`).join(',')).join('\r\n');const blob=new Blob([csv],{type:'text/csv;charset=utf-8;'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=`wcag-audit-${today.toISOString().split('T')[0]}.csv`;document.body.appendChild(a);a.click();document.body.removeChild(a);URL.revokeObjectURL(url);announce('Audit exported.');toast('Audit exported.','success');}
...
function renderWhatsNew(){
  ...
  <section class="card" style="margin-bottom:16px">
    ...
    <ul style="list-style:disc;padding-left:20px;display:flex;flex-direction:column;gap:8px;font-size:.88rem">
      <li><strong>Streamlined navigation</strong> — Primary nav now shows 7 core views (Dashboard, Tasks, Projects, Calendar, Team, Reports, WCAG Audit). Less-used views (Milestones, Project Health, Templates, Activity, What's New, Onboarding) are tucked behind a keyboard-accessible "More" toggle that auto-expands when you navigate to those views.</li>
      <li><strong>Contextual action slot</strong> — The topbar now includes a contextual area that shows view-relevant actions (Import CSV on Tasks, Print Report on WCAG Audit) alongside persistent Search, Focus Mode, and the Actions overflow menu, keeping the primary Add button always visible.</li>
      <li><strong>Actions overflow menu</strong> — Session Summary, Quick Capture, and Daily Briefing moved to an "⋮ Actions" menu in the topbar, reducing button clutter. The menu has full ArrowUp/ArrowDown/Escape keyboard navigation and correct ARIA menu roles.</li>
      <li><strong>Redesigned dashboard</strong> — Replaced the stat grid with a "Needs Attention" section: overdue tasks (sorted oldest first) and today's due tasks, each with inline Done and Edit actions. Upcoming milestones within 14 days appear below. Aging badges surface directly in the dashboard task list.</li>
      <li><strong>Progressive disclosure in task modal</strong> — Time Tracking, Comments, Attachments, and Dependencies are now collapsible <code>&lt;details&gt;</code> sections, reducing visual noise on add/edit. All sections are keyboard-accessible and announce expansion state to screen readers.</li>
      <li><strong>Filter preset toolbar</strong> — The preset bar is now a proper ARIA toolbar (<code>role="toolbar"</code>) with <code>aria-pressed</code> state on each preset button, making the active filter state visible to screen readers.</li>
      <li><strong>Bug fix: aging threshold persists on restart</strong> — The <code>agingThresholdDays</code> setting was saved to disk but not returned by the main process on startup. It now persists correctly across app restarts.</li>
    </ul>
  </section>

  <section class="card" style="margin-bottom:16px">
    <div class="section-h" style="display:flex;align-items:center;gap:10px"><span>v9.7.1</span><span style="font-size:.76rem;color:var(--muted);font-weight:400">September 2026</span></div>
    <ul style="list-style:disc;padding-left:20px;display:flex;flex-direction:column;gap:8px;font-size:.88rem">
      <li><strong>Electron security hardening</strong> — <code>sandbox:false</code> removed from BrowserWindow; the renderer now runs in the default sandboxed context. <code>aria-busy</code> removed from <code>nav()</code> to eliminate a timing race with JAWS virtual buffer rebuilds.</li>
      <li><strong>force-renderer-accessibility restored</strong> — <code>--force-renderer-accessibility=complete</code> flag re-applied after regression testing confirmed it is required for JAWS to build a full accessibility tree at launch.</li>
    </ul>
  </section>
  ...
  <section class="card" style="margin-bottom:16px">
    ...
    <ul style="list-style:disc;padding-left:20px;display:flex;flex-direction:column;gap:8px;font-size:.88rem">
      <li><strong>Two-mode welcome wizard</strong> — The setup dialog now has two tabs: <em>Digital Accessibility</em> and <em>Project Management</em>. Choose your mode once at startup to get a workspace tailored to your work.</li>
      <li><strong>Project Management mode</strong> — Selecting the PM tab hides WCAG audit tools from the entire interface. All project, task, team, calendar, and reporting features remain fully available. PM personas include Solo Project Manager, Team Lead, Program Manager, and Blank Slate, each with realistic non-accessibility sample data.</li>
      <li><strong>Switch Mode button</strong> — Onboarding Quick Setup now includes a Switch Mode button that reopens the welcome wizard, letting you change between Digital Accessibility and Project Management at any time.</li>
      <li><strong>JAWS virtual cursor fix</strong> — Main content area changed to <code>role="document"</code>, which explicitly signals to JAWS that the content area supports Browse Mode (virtual cursor navigation). Users no longer need to manually toggle Insert+Z to navigate the app.</li>
      <li><strong>Settings persistence fix</strong> — Focus Mode User name and app mode now correctly persist across restarts via the Electron settings store.</li>
    </ul>
  </section>
  ...
  <section class="card" style="margin-bottom:16px">
    ...
    <ul style="list-style:disc;padding-left:20px;display:flex;flex-direction:column;gap:8px;font-size:.88rem">
      ...
      <li><strong>Screen reader navigation fix</strong> — Removed role="application" from the whole-app wrapper. NVDA and JAWS now work in browse mode throughout the app; no manual mode switching required. role="application" is scoped only to the calendar grid where arrow-key navigation is needed.</li>
    </ul>
  </section>
...
function renderOnboarding(){
  const allPersonaLabels={tester:'Accessibility Tester',lead:'Project Lead',manager:'Project Manager',blank:'Blank Slate','pm-solo':'Solo Project Manager','pm-lead':'Team Lead','pm-manager':'Program Manager','pm-blank':'Blank Slate'};
  const personaLabel=allPersonaLabels[settings.persona]||null;
...
function switchTab(tab){
  const sectionMap={time:'section-time',comments:'section-comments',attachments:'section-attachments',dependencies:'section-dependencies'};
  ['section-time','section-comments','section-attachments','section-dependencies'].forEach(id=>{const el=document.getElementById(id);if(el)el.open=false;});
  if(tab==='details') return;
  const sid=sectionMap[tab];
  if(!sid) return;
  const sec=document.getElementById(sid);
  if(sec){sec.open=true;if(tab==='time')renderTimeLog();else if(tab==='comments')renderComments();else if(tab==='attachments')renderAttachments();else if(tab==='dependencies')renderDependencies();}
}
...
function renderAttachments(){const list=document.getElementById('attach-list');if(!list)return;if(!currentTaskAttachments.length){list.innerHTML='<p style="color:var(--muted);font-size:.84rem">No attachments yet.</p>';return;}list.innerHTML=`<ul style="list-style:none;padding:0;margin:0 0 10px">${currentTaskAttachments.map((a,i)=>`<li style="display:flex;align-items:center;gap:8px;padding:7px 10px;background:var(--surface2);border-radius:6px;margin-bottom:6px" aria-label="Attachment: ${esc(a.name)}"><span aria-hidden="true">📎</span><span style="flex:1;font-weight:600">${esc(a.name)}</span>${isSafeUrl(a.url)?`<a href="${esc(a.url)}" target="_blank" aria-label="Open ${esc(a.name)}" style="color:var(--accent);font-size:.8rem">Open</a>`:`<span style="color:var(--danger);font-size:.8rem" aria-label="Invalid URL for ${esc(a.name)}">Invalid URL</span>`}<button class="btn btn-danger btn-sm" onclick="removeAttachment(${i})" aria-label="Remove ${esc(a.name)}">✕</button></li>`).join('')}</ul>`;}
function addAttachment(){const name=document.getElementById('attach-name').value.trim(),url=document.getElementById('attach-url').value.trim();if(!name||!url){announce('Enter name and URL.');return;}if(!isSafeUrl(url)){announce('Only http and https URLs are allowed.');return;}currentTaskAttachments.push({name,url});document.getElementById('attach-name').value='';document.getElementById('attach-url').value='';renderAttachments();announce('Attachment added.');}
...
function addDependency(){if(!isProUser()){showUpgradePrompt('Task Dependencies');return;}const sel=document.getElementById('dep-select');if(!sel||!sel.value)return;const id=parseInt(sel.value);if(!currentTaskDependencies.includes(id))currentTaskDependencies.push(id);renderDependencies();announce('Dependency added.');}
...
async function saveSettings(){await AppStorage.saveSettings(pendingSettings);settingsHasChanges=false;pendingSettings={};settingsSnapshot=JSON.parse(JSON.stringify(settings));updateSaveBar();announce('Settings saved.');toast('Settings saved.','success');logActivity('Settings updated','⚙️');}
...
function renderSettingsTab(tab){if(tab==='general')return renderGeneral();if(tab==='appearance')return renderAppearance();if(tab==='accessibility')return renderAccessibility();if(tab==='drive')return renderDrive();if(tab==='shortcuts')return renderShortcuts();if(tab==='statuses')return renderStatuses();if(tab==='notifications')return renderNotifications();if(tab==='license')return renderLicense();if(tab==='account')return renderAccount();if(tab==='about')return renderAbout();return '';}
function renderLicense(){const tier=settings.tier||'free';const tLabel={free:'Free',pro:'Pro',legacy:'Legacy Pro'}[tier]||'Free';return`<div><h2 class="settings-section-title">License</h2><div class="settings-row"><div><div class="settings-row-label">Current Plan</div></div><div class="settings-row-control"><span class="badge ${tier==='free'?'badge-todo':'badge-done'}">${tLabel}</span></div></div><div class="settings-row"><div><div class="settings-row-label">License Key</div><div class="settings-row-desc">Enter your VantagePM Pro license key. Activation is coming soon.</div></div><div class="settings-row-control" style="display:flex;gap:8px;flex-wrap:wrap"><input type="text" class="form-input" style="width:230px" id="license-key-input" value="${esc(settings.licenseKey||'')}" placeholder="XXXX-XXXX-XXXX-XXXX" aria-label="License key"/><button class="btn btn-primary btn-sm" onclick="activateLicense()">Activate</button></div></div><div style="margin-top:20px;padding:16px;background:var(--surface2);border-radius:var(--radius);border:1px solid var(--border)"><div style="font-weight:700;font-size:.9rem;margin-bottom:10px"><span aria-hidden="true">⭐</span> VantagePM Pro includes:</div><ul style="font-size:.84rem;color:var(--muted);line-height:2.1;padding-left:20px"><li>Unlimited tasks and projects</li><li>Team management and workload tracking</li><li>WCAG audit export (CSV and printable report)</li><li>Time tracking and billing</li><li>Milestones and task dependencies</li><li>Unlimited templates and custom statuses</li><li>Saved filter presets and bulk actions</li><li>Google Drive sync (coming soon)</li></ul></div></div>`;}
async function activateLicense(){const key=document.getElementById('license-key-input')?.value.trim();if(!key){announce('Enter a license key.');return;}const btn=document.getElementById('license-activate-btn');if(btn){btn.disabled=true;btn.textContent='Checking…';}try{const res=await fetch(WORKER_URL+'/validate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({key})});if(!res.ok)throw new Error('Server error');const data=await res.json();if(data.valid){await saveSetting('licenseKey',key);await saveSetting('tier',data.tier||'pro');settings.tier=data.tier||'pro';settings.licenseKey=key;const body=document.getElementById('settings-panel-body');if(body)body.innerHTML=renderLicense();announce('License activated. Pro features unlocked.');toast('Pro unlocked! Welcome to VantagePM Pro.','success');}else{announce('Invalid license key. Check your key and try again.');toast('Invalid license key.','error');}}catch{announce('Could not reach the license server. Check your connection and try again.');toast('Activation failed — check your connection.','error');}finally{if(btn){btn.disabled=false;btn.textContent='Activate';}}}
function renderAccount(){
  if(typeof SupaAuth==='undefined'){return`<div><h2 class="settings-section-title">Account</h2><p style="font-size:.85rem;color:var(--muted)">Cloud accounts are not configured. Add your Supabase project URL and anon key to <strong>docs/supabase.js</strong>, then run the SQL in <strong>worker/supabase-setup.sql</strong>.</p></div>`;}
  if(_supaUser){const tier=settings.tier||'free';const tLabel={free:'Free',pro:'Pro',legacy:'Legacy Pro'}[tier]||'Free';return`<div><h2 class="settings-section-title">Account</h2><div class="settings-row"><div><div class="settings-row-label">Email</div></div><div class="settings-row-control"><span>${esc(_supaUser.email)}</span></div></div><div class="settings-row"><div><div class="settings-row-label">Plan</div></div><div class="settings-row-control"><span class="badge ${tier==='free'?'badge-todo':'badge-done'}">${tLabel}</span></div></div><div class="settings-row"><div><div class="settings-row-label">Cloud Sync</div><div class="settings-row-desc">Your data syncs automatically when changes are saved.</div></div><div class="settings-row-control"><span class="badge badge-done">Connected</span></div></div><div class="settings-row"><div><div class="settings-row-label">Upload Local Data</div><div class="settings-row-desc">Push your current device data to the cloud. Use this once after your first sign-in to migrate existing data.</div></div><div class="settings-row-control"><button class="btn btn-secondary btn-sm" onclick="supaUploadLocalData()">Upload Now</button></div></div><div class="settings-row"><div><div class="settings-row-label">Sign Out</div></div><div class="settings-row-control"><button class="btn btn-danger btn-sm" onclick="supaSignOut()">Sign Out</button></div></div></div>`;}
  const isSignup=_accountMode==='signup';
  return`<div><h2 class="settings-section-title">Account</h2><p style="font-size:.84rem;color:var(--muted);margin-bottom:16px;line-height:1.6">Create a free account to sync your data across devices and enable cloud-verified Pro access.</p><div role="tablist" aria-label="Authentication mode" style="display:flex;gap:4px;margin-bottom:20px;border-bottom:2px solid var(--border)"><button role="tab" aria-selected="${!isSignup}" class="settings-nav-btn${!isSignup?' active':''}" onclick="switchAccountMode('signin')">Sign In</button><button role="tab" aria-selected="${isSignup}" class="settings-nav-btn${isSignup?' active':''}" onclick="switchAccountMode('signup')">Create Account</button></div><div class="settings-row"><div><label class="settings-row-label" for="supa-email">Email</label></div><div class="settings-row-control"><input type="email" class="form-input" style="width:220px" id="supa-email" autocomplete="email" placeholder="you@example.com" aria-required="true"/></div></div><div class="settings-row"><div><label class="settings-row-label" for="supa-password">Password</label>${isSignup?'<div class="settings-row-desc">At least 8 characters.</div>':''}</div><div class="settings-row-control"><input type="password" class="form-input" style="width:220px" id="supa-password" autocomplete="${isSignup?'new-password':'current-password'}" aria-required="true"/></div></div><div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:16px"><button class="btn btn-primary" id="supa-auth-btn" onclick="${isSignup?'handleSignUp()':'handleSignIn()'}"><span aria-hidden="true">${isSignup?'✅':'🔓'}</span> ${isSignup?'Create Account':'Sign In'}</button><button class="btn btn-secondary" id="supa-magic-btn" onclick="handleMagicLink()"><span aria-hidden="true">✉️</span> Send Magic Link</button></div>${!isSignup?`<p style="font-size:.78rem;color:var(--muted);margin-top:12px">Forgot your password? Enter your email above and click <strong>Send Magic Link</strong>.</p>`:''}</div>`;
}
function switchAccountMode(mode){_accountMode=mode;const body=document.getElementById('settings-panel-body');if(body)body.innerHTML=renderAccount();document.getElementById('supa-email')?.focus();}
async function handleSignIn(){const email=document.getElementById('supa-email')?.value.trim();const password=document.getElementById('supa-password')?.value;if(!email||!password){announce('Enter your email and password.');return;}const btn=document.getElementById('supa-auth-btn');if(btn){btn.disabled=true;btn.textContent='Signing in…';}try{const{error}=await SupaAuth.signInWithPassword(email,password);if(error)throw error;}catch(e){announce('Sign-in failed: '+(e.message||'Unknown error'));toast('Sign-in failed: '+(e.message||'Unknown error'),'error');}finally{if(btn){btn.disabled=false;btn.textContent='Sign In';}}}
async function handleSignUp(){const email=document.getElementById('supa-email')?.value.trim();const password=document.getElementById('supa-password')?.value;if(!email||!password){announce('Enter your email and password.');return;}if(password.length<8){announce('Password must be at least 8 characters.');toast('Password must be at least 8 characters.','error');return;}const btn=document.getElementById('supa-auth-btn');if(btn){btn.disabled=true;btn.textContent='Creating account…';}try{const{error}=await SupaAuth.signUp(email,password);if(error)throw error;announce('Account created. Check your email to confirm your address.');toast('Account created — check your email to confirm.','success');}catch(e){announce('Sign-up failed: '+(e.message||'Unknown error'));toast('Sign-up failed: '+(e.message||'Unknown error'),'error');}finally{if(btn){btn.disabled=false;btn.textContent='Create Account';}}}
async function handleMagicLink(){const email=document.getElementById('supa-email')?.value.trim();if(!email){announce('Enter your email address first.');return;}const btn=document.getElementById('supa-magic-btn');if(btn){btn.disabled=true;btn.textContent='Sending…';}try{const{error}=await SupaAuth.signInWithOtp(email);if(error)throw error;announce('Magic link sent. Check your email.');toast('Magic link sent — check your email.','success');}catch(e){announce('Could not send magic link: '+(e.message||'Unknown error'));toast('Magic link failed.','error');}finally{if(btn){btn.disabled=false;btn.textContent='Send Magic Link';}}}
async function supaSignOut(){try{await SupaAuth.signOut();}catch(e){toast('Sign-out failed.','error');}}
async function supaUploadLocalData(){if(!_supaUser){announce('Sign in first.');return;}try{await SupaData.saveData({tasks,members,projects,milestones,activityLog,auditResults,customStatuses,templates,projectNotes,filterPresets,persona:settings.persona,appMode:settings.appMode});await SupaData.saveSettings(Object.assign({},settings));announce('Local data uploaded to cloud.');toast('Data synced to cloud.','success');}catch(e){toast('Upload failed — check your connection.','error');}}
function toggle(key,val,extra=''){const lbl=key.replace(/([A-Z])/g,' $1').replace(/^./,s=>s.toUpperCase()).trim();return`<label class="toggle" aria-label="${lbl}"><input type="checkbox" ${val?'checked':''} onchange="setPending('${key}',this.checked);${extra}" aria-label="${lbl}"/><span class="toggle-track" aria-hidden="true"></span><span class="toggle-thumb" aria-hidden="true"></span></label>`;}
async function saveSetting(key,value){settings[key]=value;await AppStorage.saveSettings({[key]:value});scheduleSupaSettingsSave();}
...
function testReminder(){sendWebNotification('VantagePM Test','Notifications are working!');toast('Test sent.','success');announce('Test notification sent.');}
...
async function submitFeedbackForm(){const issue=document.getElementById('fb-issue').value.trim();const impact=document.getElementById('fb-impact').value;let ok=true;if(!issue){document.getElementById('fb-issue-err').classList.remove('hidden');ok=false;}else document.getElementById('fb-issue-err').classList.add('hidden');if(!impact){document.getElementById('fb-impact-err').classList.remove('hidden');ok=false;}else document.getElementById('fb-impact-err').classList.add('hidden');if(!ok){announce('Please fix the errors above.');return;}const at=[...document.querySelectorAll('input[name="fb-at"]:checked')].map(c=>c.value);const solution=document.getElementById('fb-solution').value.trim();const btn=document.getElementById('fb-submit');btn.disabled=true;btn.textContent='Submitting…';const title='[Feedback] '+issue.slice(0,72);const bodyText='**AT:** '+(at.join(', ')||'Not specified')+'\n\n**Issue:** '+issue+'\n\n**Suggested solution:** '+(solution||'N/A')+'\n\n**Impact:** '+impact;try{const res=await fetch(WORKER_URL+'/feedback',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({title,bodyText,labels:['user-feedback']})});if(!res.ok)throw new Error('Worker error');closeModal('feedback-modal');announce('Feedback submitted. Thank you!');toast('Feedback submitted. Thank you!','success');}catch{const _ftitle=encodeURIComponent(title);const _fbody=encodeURIComponent(bodyText);window.open('https://github.com/blindgeek1989/Vantage-PM/issues/new?title='+_ftitle+'&body='+_fbody+'&labels=user-feedback','_blank');closeModal('feedback-modal');toast('Worker unavailable — GitHub opened instead.','success');announce('GitHub opened to submit your feedback.');}finally{btn.disabled=false;btn.textContent='Submit Feedback';}}
...
async function signIn(){toast('Google Drive sync is coming soon for the web version.','success');announce('Google Drive sync is coming soon.');}
async function signOut(){settings.googleConnected=false;settings.googleEmail=null;toast('Signed out.','success');announce('Signed out.');switchSettingsTab('drive');}
async function openFolderPicker(){toast('Google Drive sync is coming soon for the web version.','success');announce('Google Drive sync is coming soon.');}
...
async function syncDrive(){toast('Google Drive sync is coming soon for the web version.','success');announce('Google Drive sync is coming soon.');}
...
function checkWorkload(selectEl){
  ...
  if(open>=threshold){
    ...
    announce(`Workload warning: ${name} has ${open} open tasks.${alt?` ${alt.name} has more availability. Tab to the Assign to ${alt.name} button to reassign.`:''}`);
  } else {
...
function openBatchTimeLog(){
  if(!isProUser()){showUpgradePrompt('Time Tracking');return;}
...
function renderPresetBar(){
  if(!filterPresets.length) return `<div id="preset-bar-toolbar" class="tabs-row" role="toolbar" aria-label="Saved filter presets" onkeydown="presetToolbarKey(event)" style="padding-bottom:2px"><span style="font-size:.7rem;font-weight:700;color:var(--muted);align-self:center;padding-right:6px;white-space:nowrap;flex-shrink:0">PRESETS</span><button class="tab-btn" tabindex="0" onclick="openSavePreset()" aria-haspopup="dialog" aria-label="Save current filters as a new preset" style="color:var(--muted);font-size:.78rem"><span aria-hidden="true">＋</span> Save Preset</button></div>`;
  return`<div id="preset-bar-toolbar" class="tabs-row" role="toolbar" aria-label="Saved filter presets" onkeydown="presetToolbarKey(event)" style="padding-bottom:2px"><span style="font-size:.7rem;font-weight:700;color:var(--muted);align-self:center;padding-right:6px;white-space:nowrap;flex-shrink:0">PRESETS</span>${filterPresets.map((p,i)=>{const key=i<9?`Alt+${i+1}`:null;const active=activePresetIdx===i;return`<span style="display:inline-flex;align-items:center"><button class="tab-btn" style="${active?'color:var(--accent);border-bottom-color:var(--accent);':''}" aria-pressed="${active?'true':'false'}" tabindex="${active?'0':'-1'}" onclick="loadFilterPreset(${i})" aria-label="Preset: ${esc(p.name)}${active?' (active)':''}${key?`. Shortcut: ${key}`:''}">${esc(p.name)}${key?`<span aria-hidden="true" style="font-size:.66rem;opacity:.6;margin-left:5px">${key}</span>`:''}</button><button class="preset-del-btn" onclick="deleteFilterPreset(${i})" aria-label="Delete preset ${esc(p.name)}">✕</button></span>`;}).join('')}<button class="tab-btn" onclick="openSavePreset()" aria-haspopup="dialog" tabindex="-1" aria-label="Save current filters as a new preset" style="color:var(--muted);font-size:.78rem"><span aria-hidden="true">＋</span> Save</button></div>`;
}

function presetToolbarKey(e){
  const tb=document.getElementById('preset-bar-toolbar');
  if(!tb)return;
  const items=[...tb.querySelectorAll('button')].filter(b=>!b.disabled&&b.offsetParent!==null);
  const idx=items.indexOf(document.activeElement);
  if(idx===-1)return;
  if(e.key==='ArrowRight'||e.key==='ArrowDown'){e.preventDefault();const n=items[(idx+1)%items.length];items.forEach(b=>b.tabIndex=-1);n.tabIndex=0;n.focus();}
  else if(e.key==='ArrowLeft'||e.key==='ArrowUp'){e.preventDefault();const n=items[(idx-1+items.length)%items.length];items.forEach(b=>b.tabIndex=-1);n.tabIndex=0;n.focus();}
  else if(e.key==='Home'){e.preventDefault();items.forEach(b=>b.tabIndex=-1);items[0].tabIndex=0;items[0].focus();}
  else if(e.key==='End'){e.preventDefault();items.forEach(b=>b.tabIndex=-1);items[items.length-1].tabIndex=0;items[items.length-1].focus();}
}
function openSavePreset(){
  if(!isProUser()){showUpgradePrompt('Saved Filter Presets');return;}
...
function toggleRowMenu(e,id){e.stopPropagation();const menu=document.getElementById('row-menu-'+id);const btn=document.getElementById('row-menu-btn-'+id);const wasOpen=!menu.hidden;closeAllRowMenus();if(!wasOpen){menu.hidden=false;btn.setAttribute('aria-expanded','true');menu.onkeydown=ev=>menuKeyNav(ev,menu,btn);const first=menu.querySelector('[role=menuitem]');if(first)first.focus();}}
function menuKeyNav(e,menu,triggerBtn){const items=[...menu.querySelectorAll('[role=menuitem]')];const cur=items.indexOf(document.activeElement);if(e.key==='ArrowDown'){e.preventDefault();items[(cur+1)%items.length].focus();}else if(e.key==='ArrowUp'){e.preventDefault();items[(cur-1+items.length)%items.length].focus();}else if(e.key==='Home'){e.preventDefault();items[0].focus();}else if(e.key==='End'){e.preventDefault();items[items.length-1].focus();}else if(e.key==='Tab'||e.key==='Escape'){e.preventDefault();closeAllRowMenus();triggerBtn.focus();}}
function closeAllRowMenus(){document.querySelectorAll('.row-menu').forEach(m=>{m.hidden=true;const id=m.id.replace('row-menu-','');const btn=document.getElementById('row-menu-btn-'+id);if(btn)btn.setAttribute('aria-expanded','false');});closeActionsMenu();}
...
function switchOnboardingMode(mode){settings.appMode=mode;AppStorage.saveSettings({appMode:mode});applyAppMode();nav('onboarding',document.querySelector('[data-view=onboarding]'));announce(mode==='pm'?'Project Management mode selected. Role options updated.':'Digital Accessibility mode selected. Role options updated.');}
...
function openSetupModal(){
  ...
  activeSetupTab=tab;
  switchSetupTab(tab);
  document.querySelectorAll('input[name="setup-role"],input[name="setup-role-pm"]').forEach(r=>r.checked=false);
  if(settings.persona){
    const pmTypes=['pm-solo','pm-lead','pm-manager','pm-blank'];
    const radioName=pmTypes.includes(settings.persona)?'setup-role-pm':'setup-role';
    const ex=document.querySelector(`input[name="${radioName}"][value="${settings.persona}"]`);
...
function completeSetup(){
  const radioName=activeSetupTab==='pm'?'setup-role-pm':'setup-role';
  const errId=activeSetupTab==='pm'?'setup-role-pm-err':'setup-role-a11y-err';
  const roleEl=document.querySelector(`input[name="${radioName}"]:checked`);
  const name=(document.getElementById('setup-name')?.value||'').trim();
  let ok=true;
  if(!roleEl){document.getElementById(errId).classList.remove('hidden');ok=false;}
  else document.getElementById(errId).classList.add('hidden');
  ...
  settings.appMode=activeSetupTab==='pm'?'pm':'accessibility';
  AppStorage.saveSettings({focusModeUser:name,hasSeenOnboarding:true,appMode:settings.appMode});
...
function doSetupPersona(type){
  ...
  } else if(type==='pm-solo'){
    const proj={id:nextProjectId++,name:'Website Redesign',desc:'Full redesign of the company website',color:'#1547C8'};
    projects.push(proj);
    members.push({id:nextMemberId++,name:you,email:'',role:'Project Manager',dept:'',photo:''});
    tasks.push({id:nextTaskId++,name:'Define project scope',desc:'Document goals, deliverables, constraints, and stakeholder expectations.',assignee:you,priority:'high',status:'inprogress',due:d(3),progress:30,projectId:proj.id,recur:'none',colorLabel:'#1E88E5',comments:[],attachments:[],dependencies:[],timeLog:[],pinned:true});
    tasks.push({id:nextTaskId++,name:'Gather stakeholder requirements',desc:'Interview key stakeholders and document functional and non-functional requirements.',assignee:you,priority:'high',status:'todo',due:d(7),progress:0,projectId:proj.id,recur:'none',colorLabel:'',comments:[],attachments:[],dependencies:[],timeLog:[],pinned:false});
    tasks.push({id:nextTaskId++,name:'Create wireframes',desc:'Produce low-fidelity wireframes for all primary page layouts.',assignee:you,priority:'medium',status:'todo',due:d(14),progress:0,projectId:proj.id,recur:'none',colorLabel:'',comments:[],attachments:[],dependencies:[],timeLog:[],pinned:false});
    tasks.push({id:nextTaskId++,name:'Design review',desc:'Present designs to stakeholders and capture feedback for revisions.',assignee:you,priority:'medium',status:'todo',due:d(21),progress:0,projectId:proj.id,recur:'none',colorLabel:'',comments:[],attachments:[],dependencies:[],timeLog:[],pinned:false});
    tasks.push({id:nextTaskId++,name:'Development kickoff',desc:'Brief the dev team on requirements, designs, and timeline.',assignee:you,priority:'high',status:'todo',due:d(28),progress:0,projectId:proj.id,recur:'none',colorLabel:'',comments:[],attachments:[],dependencies:[],timeLog:[],pinned:false});
    templates.push({id:nextTemplateId++,name:'Project Kickoff Task',data:{desc:'Goal: \nDeliverables: \nOwner: \nDeadline: ',priority:'high',status:'todo',recur:'none',colorLabel:'#1E88E5',projectId:proj.id}});
    filterPresets=[{name:'In Progress',status:'inprogress',priority:'all',project:String(proj.id),search:''},{name:'High Priority',status:'all',priority:'high',project:'all',search:''}];
    settings.persona='pm-solo';
  } else if(type==='pm-lead'){
    const p1={id:nextProjectId++,name:'Website Redesign',desc:'Full redesign of the company website',color:'#1547C8'};
    const p2={id:nextProjectId++,name:'Brand Refresh',desc:'Logo, color palette, and brand guide update',color:'#0A6B3C'};
    projects.push(p1,p2);
    members.push({id:nextMemberId++,name:you,email:'',role:'Team Lead',dept:'',photo:''});
    members.push({id:nextMemberId++,name:'Alex Rivera',email:'alex@example.com',role:'UX Designer',dept:'Design',photo:''});
    members.push({id:nextMemberId++,name:'Jordan Kim',email:'jordan@example.com',role:'Front-end Developer',dept:'Engineering',photo:''});
    members.push({id:nextMemberId++,name:'Sam Patel',email:'sam@example.com',role:'Content Strategist',dept:'Marketing',photo:''});
    tasks.push({id:nextTaskId++,name:'Finalize homepage layout',desc:'Agree on grid structure, hero section, and navigation pattern.',assignee:'Alex Rivera',priority:'high',status:'inprogress',due:d(3),progress:60,projectId:p1.id,recur:'none',colorLabel:'#1E88E5',comments:[{author:'Alex Rivera',text:'Hero section is locked in — working on nav now.',timestamp:new Date().toISOString()}],attachments:[],dependencies:[],timeLog:[{person:'Alex Rivera',hours:4,note:'Hero and grid layout done',date:new Date().toISOString()}],pinned:true});
    tasks.push({id:nextTaskId++,name:'Write copy for key pages',desc:'Home, About, Services, and Contact page copy.',assignee:'Sam Patel',priority:'high',status:'todo',due:d(5),progress:0,projectId:p1.id,recur:'none',colorLabel:'',comments:[],attachments:[],dependencies:[],timeLog:[],pinned:false});
    tasks.push({id:nextTaskId++,name:'Build component library',desc:'Reusable button, card, form, and nav components in the new brand.',assignee:'Jordan Kim',priority:'medium',status:'todo',due:d(10),progress:0,projectId:p1.id,recur:'none',colorLabel:'',comments:[],attachments:[],dependencies:[],timeLog:[],pinned:false});
    tasks.push({id:nextTaskId++,name:'Sprint review prep',desc:'Compile all completed items with before/after screenshots.',assignee:you,priority:'medium',status:'todo',due:d(14),progress:0,projectId:p1.id,recur:'none',colorLabel:'',comments:[],attachments:[],dependencies:[],timeLog:[],pinned:false});
    tasks.push({id:nextTaskId++,name:'New logo concepts',desc:'Three logo directions for stakeholder review.',assignee:'Alex Rivera',priority:'high',status:'todo',due:d(7),progress:0,projectId:p2.id,recur:'none',colorLabel:'#E53935',comments:[],attachments:[],dependencies:[],timeLog:[],pinned:false});
    tasks.push({id:nextTaskId++,name:'Color palette selection',desc:'Evaluate five palette options against brand values and print/digital needs.',assignee:'Sam Patel',priority:'medium',status:'todo',due:d(12),progress:0,projectId:p2.id,recur:'none',colorLabel:'',comments:[],attachments:[],dependencies:[],timeLog:[],pinned:false});
    milestones.push({id:nextMilestoneId++,name:'Website Beta Launch',date:d(21),projectId:p1.id,desc:'Beta version live for stakeholder review'});
    milestones.push({id:nextMilestoneId++,name:'Brand Guide Delivery',date:d(28),projectId:p2.id,desc:'Final brand guide delivered to all teams'});
    templates.push({id:nextTemplateId++,name:'Design Review Task',data:{desc:'Component: \nFeedback: \nRevisions needed: ',priority:'medium',status:'todo',recur:'none',colorLabel:'#1E88E5',projectId:p1.id}});
    filterPresets=[{name:'Website Tasks',status:'all',priority:'all',project:String(p1.id),search:''},{name:'Brand Tasks',status:'all',priority:'all',project:String(p2.id),search:''},{name:'High Priority',status:'all',priority:'high',project:'all',search:''}];
    settings.persona='pm-lead';
  } else if(type==='pm-manager'){
    const p1={id:nextProjectId++,name:'Product Launch Q3',desc:'New product line launch — go-to-market and launch events',color:'#1547C8'};
    const p2={id:nextProjectId++,name:'Website Redesign',desc:'Full company website redesign and rebuild',color:'#0A6B3C'};
    const p3={id:nextProjectId++,name:'Marketing Campaign',desc:'Q3 digital and print marketing campaign',color:'#7A4700'};
    projects.push(p1,p2,p3);
    members.push({id:nextMemberId++,name:you,email:'',role:'Program Manager',dept:'',photo:''});
    members.push({id:nextMemberId++,name:'Morgan Lee',email:'morgan@example.com',role:'Product Manager',dept:'Product',photo:''});
    members.push({id:nextMemberId++,name:'Taylor Ross',email:'taylor@example.com',role:'UX Designer',dept:'Design',photo:''});
    members.push({id:nextMemberId++,name:'Casey Nguyen',email:'casey@example.com',role:'Front-end Developer',dept:'Engineering',photo:''});
    members.push({id:nextMemberId++,name:'Drew Chen',email:'drew@example.com',role:'Marketing Manager',dept:'Marketing',photo:''});
    tasks.push({id:nextTaskId++,name:'Stakeholder alignment meeting',desc:'Align all department heads on Q3 product launch scope and timeline.',assignee:you,priority:'high',status:'done',due:d(-7),progress:100,projectId:p1.id,recur:'none',colorLabel:'#43A047',comments:[],attachments:[],dependencies:[],timeLog:[{person:you,hours:2,note:'All stakeholders aligned — green light given',date:new Date(Date.now()-7*86400000).toISOString()}],pinned:false});
    tasks.push({id:nextTaskId++,name:'Define Q3 OKRs',desc:'Set measurable launch targets: units, press coverage, demo signups.',assignee:you,priority:'high',status:'done',due:d(-5),progress:100,projectId:p1.id,recur:'none',colorLabel:'#43A047',comments:[],attachments:[],dependencies:[],timeLog:[{person:you,hours:3,note:'OKRs finalized and distributed',date:new Date(Date.now()-5*86400000).toISOString()}],pinned:true});
    tasks.push({id:nextTaskId++,name:'Launch event planning',desc:'Venue, catering, AV, speakers, and run-of-show for the launch event.',assignee:'Morgan Lee',priority:'high',status:'inprogress',due:d(7),progress:45,projectId:p1.id,recur:'none',colorLabel:'#1E88E5',comments:[],attachments:[],dependencies:[],timeLog:[{person:'Morgan Lee',hours:6,note:'Venue booked, catering TBD',date:new Date().toISOString()}],pinned:false});
    tasks.push({id:nextTaskId++,name:'Press kit preparation',desc:'One-pager, product photos, key messages, and spokesperson bios.',assignee:'Drew Chen',priority:'medium',status:'todo',due:d(14),progress:0,projectId:p1.id,recur:'none',colorLabel:'',comments:[],attachments:[],dependencies:[],timeLog:[],pinned:false});
    tasks.push({id:nextTaskId++,name:'Homepage redesign',desc:'New homepage reflecting Q3 product line and brand refresh.',assignee:'Taylor Ross',priority:'high',status:'inprogress',due:d(10),progress:50,projectId:p2.id,recur:'none',colorLabel:'#1E88E5',comments:[],attachments:[],dependencies:[],timeLog:[{person:'Taylor Ross',hours:8,note:'Wireframes approved, moving to high-fidelity',date:new Date().toISOString()}],pinned:false});
    tasks.push({id:nextTaskId++,name:'CMS migration',desc:'Migrate 200 pages from legacy CMS to new headless platform.',assignee:'Casey Nguyen',priority:'high',status:'todo',due:d(21),progress:0,projectId:p2.id,recur:'none',colorLabel:'',comments:[],attachments:[],dependencies:[],timeLog:[],pinned:false});
    tasks.push({id:nextTaskId++,name:'Performance audit',desc:'Lighthouse audit on all key pages — target 90+ performance score.',assignee:'Casey Nguyen',priority:'medium',status:'done',due:d(-3),progress:100,projectId:p2.id,recur:'none',colorLabel:'#43A047',comments:[],attachments:[],dependencies:[],timeLog:[{person:'Casey Nguyen',hours:2,note:'All pages 92+ — report filed',date:new Date(Date.now()-3*86400000).toISOString()}],pinned:false});
    tasks.push({id:nextTaskId++,name:'Social media campaign brief',desc:'Creative brief for Q3 paid and organic social across LinkedIn, Instagram, and X.',assignee:'Drew Chen',priority:'medium',status:'done',due:d(-4),progress:100,projectId:p3.id,recur:'none',colorLabel:'#43A047',comments:[],attachments:[],dependencies:[],timeLog:[{person:'Drew Chen',hours:3,note:'Brief approved by creative director',date:new Date(Date.now()-4*86400000).toISOString()}],pinned:false});
    tasks.push({id:nextTaskId++,name:'Ad creative production',desc:'Banner ads, social assets, and email header images for Q3 campaign.',assignee:'Taylor Ross',priority:'high',status:'inprogress',due:d(6),progress:65,projectId:p3.id,recur:'none',colorLabel:'#1E88E5',comments:[],attachments:[],dependencies:[],timeLog:[{person:'Taylor Ross',hours:5,note:'Social assets done, working on banners',date:new Date().toISOString()}],pinned:false});
    tasks.push({id:nextTaskId++,name:'Email campaign setup',desc:'Build and QA three email flows in the marketing platform.',assignee:'Drew Chen',priority:'medium',status:'todo',due:d(12),progress:0,projectId:p3.id,recur:'none',colorLabel:'',comments:[],attachments:[],dependencies:[],timeLog:[],pinned:false});
    milestones.push({id:nextMilestoneId++,name:'Q3 Product Launch',date:d(28),projectId:p1.id,desc:'Public launch event and press announcement'});
    milestones.push({id:nextMilestoneId++,name:'New Website Live',date:d(14),projectId:p2.id,desc:'Redesigned website goes live'});
    milestones.push({id:nextMilestoneId++,name:'Campaign Go-Live',date:d(10),projectId:p3.id,desc:'Q3 campaign live across all channels'});
    filterPresets=[{name:'My Tasks',status:'all',priority:'all',project:'all',search:you},{name:'In Progress',status:'inprogress',priority:'all',project:'all',search:''},{name:'High Priority',status:'all',priority:'high',project:'all',search:''},{name:'Launch',status:'all',priority:'all',project:String(p1.id),search:''}];
    settings.persona='pm-manager';
  } else {
    settings.persona=type==='pm-blank'?'pm-blank':'blank';
  }

  scheduleLocalSave();
  AppStorage.saveSettings({persona:settings.persona,appMode:settings.appMode});
  const labels={tester:'Accessibility Tester',lead:'Project Lead',manager:'Project Manager',blank:'Blank Slate','pm-solo':'Solo Project Manager','pm-lead':'Team Lead','pm-manager':'Program Manager','pm-blank':'Blank Slate (PM)'};
