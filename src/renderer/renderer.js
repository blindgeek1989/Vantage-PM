...
function scheduleLocalSave(){
  ...
  _localSaveTimer=setTimeout(()=>window.electronAPI.localSave({tasks,members,projects,milestones,activityLog,auditResults,customStatuses,templates,projectNotes,filterPresets,persona:settings.persona,appMode:settings.appMode}),800);
}
...
function completeSetup(){
  ...
  if(IS_ELECTRON)window.electronAPI.saveSettings({focusModeUser:name,hasSeenOnboarding:true,appMode:settings.appMode});
...
function doSetupPersona(type){
  ...
  scheduleLocalSave();
  if(IS_ELECTRON)window.electronAPI.saveSettings({persona:settings.persona,appMode:settings.appMode});
