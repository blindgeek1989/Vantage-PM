...
function nav(view,btn,settingsTab){
  ...
  requestAnimationFrame(()=>{root.removeAttribute('aria-busy');announce(`Navigated to ${title}.`);setTimeout(()=>{const h=root.querySelector('h1,h2,h3');if(h){if(!h.hasAttribute('tabindex'))h.setAttribute('tabindex','-1');h.focus();}else root.focus();},100);});
}
