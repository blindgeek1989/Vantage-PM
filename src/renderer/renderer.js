...
const DEFAULT_SETTINGS = {
  ...
  googleConnected:false,googleEmail:null,platform:'win32',appVersion:'8.1.0',agingThresholdDays:5,
};
...
function saveProject(){const name=document.getElementById('p-name').value.trim();if(!name){announce('Enter a project name.');return;}projects.push({id:nextProjectId++,name,desc:document.getElementById('p-desc').value.trim(),color:document.getElementById('p-color').value});logActivity(`Project "${name}" created`,'📁');closeModal('project-modal');nav('projects',document.querySelector('[data-view=projects]'));announce('Project added.');toast('Project added.','success');}
...
function renderOnboarding(){
  ...
  const canChangeRole=settings.persona==='lead'||settings.persona==='manager';
...
function applyOnboardingPersona(){const el=document.querySelector('input[name="onboard-role"]:checked');if(!el){announce('Please select a role first.');return;}setupPersona(el.value);}
